...
def test_auth_semantics_lookalike_override():
    ...
    assert res.is_lookalike_authenticated is True
    assert "lookalike" in res.establishes.lower()
    assert "NOT establish" in res.does_not_establish
    assert "deceptive infrastructure" in res.investigation_effect.lower()


def test_auth_semantics_forwarding_scenario():
    res = evaluate_auth_semantics(
        spf="fail",
        dkim="pass",
        dmarc="pass",
    ...
    assert res.forwarding_detected is True
    assert "forwarding chain" in res.establishes.lower()


# Test F1: Evidence Conflicts (Cross-evidence contradictions)
def test_evidence_conflict_auth_pass_vs_content_threat():
    ...
    assert "auth_pass_vs_content_risk" in conflict_types
    # Verifies dual-sided evidence citation
    match = next(c for c in conflicts if c["conflict_type"] == "auth_pass_vs_content_risk")
    assert "Protocol Authentication" in match["evidence_side_a"]
    assert "Content Threat Engine" in match["evidence_side_b"]


def test_evidence_conflict_anchor_display_mismatch():
    mismatch_url = {
    ...
    conflict_types = [c["conflict_type"] for c in conflicts]
    assert "display_text_vs_destination_host" in conflict_types


# Test F4: First-Contact Novelty Guardrail Assertion
def test_first_contact_guardrail_suppression():
    ...
    assert final_score == 48  # 58 - 10 points
    assert verdict == "Elevated"  # Dropped from High to Elevated


# Test F8: Weighted Additive Scorer & Ceiling Clamping
def test_explainable_scoring_ceilings():
    ...
    assert content_points <= 35
    assert verdict.score <= 35
    assert "legal determination" in verdict.disclaimer


# Test F6: Structural HTML Fingerprinting
def test_structural_html_fingerprint_invariant():
    html_template_1 = """
    <html>
        <body>
    ...
    assert skel_1 == skel_2


# Test M9 Static Attachment Analyzer
def test_attachment_static_analysis_disguised_executable():
    ...
    assert res["true_mime"] == "application/x-dosexec"
    assert res["is_suspicious"] is True
    assert any(ind["type"] == "disguised_executable" for ind in res["static_indicators"])
