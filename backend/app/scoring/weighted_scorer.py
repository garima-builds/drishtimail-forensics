...
def compute_explainable_score(
    ...
    ceilings = custom_ceilings or FAMILY_CEILINGS
    ...
    if conflict_pts != 0:
        contributions.append({
            ...
            "family": "conflicts",
            "points": conflict_pts,
            "reason": f"Adjusted score based on {len(conflicts)} cross-evidence contradictions.",
    ...
    # 3. Check for hard threat signals
    ...
    return ScoredVerdict(
        ...
        contributions=sorted(contributions, key=lambda x: abs(x["points"]), reverse=True),
